...
namespace MyMobiz.Controllers
{
    ...
    public class QuotesController : ControllerBase
    {
        ...
        public async Task<ActionResult<DTCalculateQuote>> CalculateQuoteTasksAsync(DTCalculateQuote dtCalculateQuote)
        {
            ...
            quote.RefererId = _context.Referers.FirstOrDefault(e => e.ServiceId == dtCalculateQuote.ServiceID).Id;
        ...
        { 
            using (var scope = _scopeFactory.CreateScope())
            {
                ...
                for(int i = 0; i < dtCalculateQuote.Journey.Count; i++)
                {
                    ...
                    await context.Places.AddAsync(orderedPlaces[i]);
                }
                ...
                for (int i=0; i < dtCalculateQuote.Legs.Count-extraLeg; i++)
                {
                    dtCalculateQuote.Legs[i+ extraLeg].FromPlaceId = orderedPlaces[i].Id;
                    dtCalculateQuote.Legs[i+ extraLeg].ToPlaceId = orderedPlaces[i + 1].Id;
                    await context.Legs.AddAsync(dtCalculateQuote.Legs[i+ extraLeg]); //Adding Leg to Context
                }
                ...
                for (int i = 0; i < dtCalculateQuote.Legs.Count-extraLeg; i++)
                {
                    ...
                    ridesLegs[i].LegId = dtCalculateQuote.Legs[i+extraLeg].Id;
        ...
        {
            Servicerates rates = await _context.Servicerates.Where(s => s.AppDate <= DateTime.Today && s.ServiceId == dtCalculateQuote.ServiceID).OrderByDescending(s => s.AppDate).FirstOrDefaultAsync();
            decimal price;
            ...
            else
            {
                ...
                for(int i = 0; i < dtCalculateQuote.Categories.Count; i++)
                {
                    ...
                    {
                        foreach (Ratesdetails detail in _context.Ratesdetails.Where(e => e.Vernum == rates.VerNum && e.CategoryId == dtCalculateQuote.Categories[i].Category).ToList())
                        {
                ...
                for(int i=0; i < totalPrice.Count; i++)
                {
                    switch (totalPrice[i].op)
                    {
                        ...
                        case "*":
                            ...
                            break;
                        case "%":
                            price = ((totalPrice[i].Figure / 100) * price) + price;
                            break;
                        case "=":
                            price = totalPrice[i].Figure;
                            break;
